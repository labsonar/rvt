...
class Validate():
    ...
    def __init__(self, files: typing.List[str], detectors: typing.List[Detector], root:str = "Result"):
    ...
    def build_table(self, metrics_list: typing.List[metric.Metric]) -> pd.DataFrame:
        ...
        for detector in self.data.index:
            for metric_ in metrics_list:
                ...
                table.loc[detector, str(metric_)] = f"{np.mean(values):.2f} +- {np.std(values):.2f}%"
    ...
    def confusion_matrix(self, detector: str) -> None:
        ...
        fig, ax = plt.subplots()
        cax = ax.matshow(cm_mean, cmap="Oranges")
